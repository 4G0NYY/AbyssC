...
pub fn compress(inputs: &[PathBuf], dest: &Path, opts: &CodecOptions) -> io::Result<()> {
    ...
    for input in inputs {
        ...
        add_path(&mut zip, input, base, options)?;
    }
...
fn add_path<W: Write + Seek>(
    ...
    if meta.is_dir() {
        ...
        for entry in fs::read_dir(disk_path)? {
            ...
            add_path(zip, &entry.path(), &format!("{arch_name}/{child}"), options)?;
        }
    } else {
        zip.start_file(arch_name.to_string(), options)?;
        let mut src = BufReader::with_capacity(1 << 20, File::open(disk_path)?);
        io::copy(&mut src, zip)?;
    }
    Ok(())
}
...
pub fn decompress(src: &Path, out_dir: &Path) -> io::Result<()> {
